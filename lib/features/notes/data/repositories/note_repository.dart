...
class IsarNoteRepository {
  ...
  Future<void> saveNote(domain.Note note) async {
    ...
    for (final b in note.blocks) {
      if (b is domain.TextBlock) {
         encryptedBlocks.add(b.copyWith(data: await _securityService.encryptData(b.data)));
  ...
  Stream<List<domain.Note>> watchAllNotes() {
    return _isar.isarNotes.where().watch(fireImmediately: true).asyncMap((isarNotes) async {
      ...
      for (final isarNote in isarNotes) {
         ...
         for (final b in domainNote.blocks) {
            if (b is domain.TextBlock) {
              decryptedBlocks.add(b.copyWith(data: await _securityService.decryptData(b.data)));
            } else if (b is domain.CheckboxBlock) {
              decryptedBlocks.add(b.copyWith(data: await _securityService.decryptData(b.data)));
            } else {
         ...
         decryptedNotes.add(domainNote.copyWith(
            title: title,
            content: content,
